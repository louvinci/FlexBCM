...
def BCMU_resource(alloc_layers,hw_params):
    ...
    return FFT_dsp+D_arrary+quant_dsp,Axi_BRAM+inbuf+obuf
...
def test(cfg, log_dir):
    ...
    import time
    blk_indx = np.where(BLK == cfg.blk_size)[0] 
    ...
    layers_param = np.array([[Cin,Cout,Hin,Win,K,S,Pad,blk_indx]])
...
if __name__ == "__main__":
    ...
    #                 else:
    #                     cfg.pad = 0
    #                 logdir = os.path.join(predir,'simulator_blk{0}-2.log'.format(tblk))
    #                 test(cfg,logdir)
