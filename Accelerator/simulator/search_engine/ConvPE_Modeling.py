...
def ConvPE_Resource(tile_param, K_max, S_max, bw_param):
    ...
    out_buf = BRAM_CONSUME(4*BIT, tr * tc) * tm
...
def CE_Latency(tile_param, N, K, S, in_bw, wt_bw):
    ...
    depth = 3
    wt_lat = depth + DDRLatency + np.ceil(K * K * tm * tn / wt_num_cycle)

    depth = 12
    conv_lat = depth + K * K * tr * tc

    fill_lat = tr * tc
    in_wt_lat = max(in_lat, wt_lat)
    loop_lat = max(in_wt_lat, conv_lat)
