...
def GA_Engine_C2D(layers, DSP, BRAM, BW, NIND=200, MAX_GEN=20):
...
def GA_Engine_BCM(layers, DSP, BRAM, BW, NIND=200, MAX_GEN=100):
...
class GAProblem_All(ea.Problem):
    def __init__(self, DSP, BRAM, BW, C2D_layers, BCM_layers):
        ...
        varTypes = [1] * Dim        # 0:continuous,1:Discrete
        lb = [0] * Dim
        ub = [3, 4, 9, 6, 6, 5, 5, 3, 4, 8, 5, 7, 5]    # index (num of selection)
        lbin = [1] * Dim            # 0 not include the bound,1 include the bound
        ubin = [0] * Dim            # index range:0~ub-1
        self.DSP = DSP
        self.BRAM = BRAM
...
def evaluate_All(args):
    ...
    in_bw, wt_bw = C2D_SEARCH['in_bw'][in_bw_id], C2D_SEARCH['wt_bw'][wt_bw_id]
    out_bw, bn_bw = C2D_SEARCH['out_bw'][out_bw_id], C2D_SEARCH['bn_bw'][bn_bw_id]
    C2D_bram, C2D_dsp, C2D_bw, C2D_layers_lat = ConvPE_Modeling([tr, tr, tm, tn], C2D_layers, [in_bw, wt_bw, out_bw, bn_bw])
    ...
    (BCM_dsp, BCM_bram), BCM_layers_lat = BCMPU_modeling(BCM_layers, [tr_b, tr_b, tm_b, tn_b, in_bw_b, wt_bw_b, out_bw_b])

    dsp = C2D_dsp + BCM_dsp
    bram = C2D_bram + BCM_bram
    total_bw = in_bw + wt_bw + out_bw * 2 + bn_bw + in_bw_b + wt_bw_b + out_bw_b
    lat = sum(C2D_layers_lat) + sum(BCM_layers_lat)
    return dsp, bram, total_bw, lat
...
def GA_Engine_All(C2D_layers, BCM_layers, DSP, BRAM, BW, NIND=200, MAX_GEN=100):
    ...
    myAlgorithm.mutOper.Pm = 0.7        # Variation probability
    myAlgorithm.drawing = 0
    myAlgorithm.logTras = 20            # 20 # 0: no log info, >0: interval gerneration one log
    myAlgorithm.verbose = True          # True # # print log
    [BestIndi, population] = myAlgorithm.run()
...
if __name__ == "__main__":
    ...
    tile_param, lat = GA_Engine_C2D(alloc_layers, 500, 200, 64)
